...
### v3.3 (2026-01-07)
- **自動ぼかしの精度向上**: 被写体（人物）をより広く保護するようにチューニングを行いました。背景として認識される範囲が厳密になり、服や髪の毛が巻き込まれにくくなりました。
- **ぼかし範囲の可視化**: 自動ぼかし実行時および手動ぼかしモード時に、ぼかされる範囲（背景）が一時的に**赤色**で表示されるようになりました。どこがボケるかが一目で確認できます。
- **バグ修正**: 自動ぼかしが画面全体にかかってしまう問題を修正しました。

### v3.2 (2026-01-06)
- **4種類の絵文字に対応**: 標準の笑顔(☺️)に加え、にっこり(😊)、爆笑(😂)、青ざめ(😨)を選べるようになりました。
- **クリップボードへコピー**: 画像を保存せずに、直接クリップボードにコピーするボタンを追加しました。
- **UI/UXの改善**:
    - ボタンを押した後の「保存しました」等のフィードバックをボタン内に表示するように変更。
    - 「囲ってぼかす」機能使用時のレイアウトズレを修正。
    - モバイル表示でのレイアウト崩れを防止。
- **バグ修正**: 白塗り・黒塗りマスクの変形時にサイズがずれる問題を修正しました。
